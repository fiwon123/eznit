...
func (config *Config) GetUser(id string) *model.User {
	...
	row := config.sqlDB.QueryRow(query, id)
...
func (config *Config) UserExists(email string) bool {
	...
	err := config.sqlDB.QueryRow(query, email).Scan(&count)
...
func (config *Config) CreateUser(user model.User) bool {
	...
	_, err := config.sqlDB.Exec(exec, user.Email, user.Password)
	if err != nil {
		fmt.Println(err)
		return false
	}
...
func (config *Config) DeleteUser(user model.User) bool {
	...
	_, err := config.sqlDB.Exec(exec, user.ID)
	if err != nil {
		fmt.Println(err)
		return false
	}
...
func (config *Config) UpdateUser(user model.User) bool {
	...
	_, err := config.sqlDB.Exec(exec, user.ID, user.Email, user.Password)
	if err != nil {
		fmt.Println(err)
		return false
	}
